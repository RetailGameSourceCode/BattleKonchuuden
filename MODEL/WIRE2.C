...
#define hide_line	(0)				/* 陰線処理(使用禁止) */
#define back_clip	(0)				/* バッククリップ */
#define light_calc	(0)				/* 光源計算 */
#define normal_draw	(0)				/* 法線表示 */
#define vertex_draw	(0)				/* 法線表示 */
#define norm_len	(256)			/* 法線長さ(1/n) */
//						ディザ    色数     半透明
#define ALIGHT_TPAGE	((1 << 9)|(0 << 7)|(1 << 5))

/**************************************************************************
	PolyF3 
 ***************************************************************************/
PACKET *Wire2F3(op, vp, np, pk, n, shift, ot)
	TMD_P_F3 *op;
	VERT   *vp, *np;
	PACKET *pk;
	int     n, shift;
	GsOT   *ot;
{
	...
	for (i = 0; i < n; i++, op++) {
		...
		*tag = ((u_long) &si_env[7] << 8) >> 8;
		si_env += 12;
#else
		si_env[7] = (u_long)((((u_long) &si_env[0] << 8) >> 8) | 0x01000000);
		si_env[8] = (u_long)((0xe1 << 24) | ALIGHT_TPAGE);
...
/**************************************************************************
	PolyF4 
 ***************************************************************************/
PACKET *Wire2F4(op, vp, np, pk, n, shift, ot)
	TMD_P_F4 *op;
	VERT   *vp, *np;
	PACKET *pk;
	int     n, shift;
	GsOT   *ot;
{
	...
	for (i = 0; i < n; i++, op++) {
		...
		*tag = ((u_long) &si_env[11] << 8) >> 8;
		si_env += 17;
#else
		si_env[7] = (u_long)((((u_long) &si_env[9] << 8) >> 8) | 0x01000000);
		si_env[8] = (u_long)((0xe1 << 24) | ALIGHT_TPAGE);
		si_env[9] = (u_long)((((u_long) &si_env[0] << 8) >> 8) | 0x03000000);
		si_env[10] = ((0x42 << 24) | ((si_env[1] << 8) >> 8));
		si_env[11] = si_env[2];
		si_env[12] = si_env[5];
		*si_env = (u_long)((( *tag << 8) >> 8) | 0x06000000);
		*tag = ((u_long) &si_env[7] << 8) >> 8;
...
/**************************************************************************
	PolyG3 
 ***************************************************************************/
PACKET *Wire2G3(op, vp, np, pk, n, shift, ot)
	TMD_P_G3 *op;
	VERT   *vp, *np;
	PACKET *pk;
	int     n, shift;
...
/**************************************************************************
	PolyG4 
 ***************************************************************************/
PACKET *Wire2G4(op, vp, np, pk, n, shift, ot)
	TMD_P_G4 *op;
	VERT   *vp, *np;
	PACKET *pk;
	int     n, shift;
...
/**************************************************************************
	PolyFT3 
 ***************************************************************************/
PACKET *Wire2FT3(op, vp, np, pk, n, shift, ot)
	TMD_P_TF3 *op;
	VERT   *vp, *np;
	PACKET *pk;
	int     n, shift;
...
/**************************************************************************
	PolyFT4 
 ***************************************************************************/
PACKET *Wire2FT4(op, vp, np, pk, n, shift, ot)
	TMD_P_TF4 *op;
	VERT   *vp, *np;
	PACKET *pk;
	int     n, shift;
...
/**************************************************************************
	PolyGT3 
 ***************************************************************************/
PACKET *Wire2GT3(op, vp, np, pk, n, shift, ot)
	TMD_P_TG3 *op;
	VERT   *vp, *np;
	PACKET *pk;
	int     n, shift;
...
/**************************************************************************
	PolyGT4 
 ***************************************************************************/
PACKET *Wire2GT4(op, vp, np, pk, n, shift, ot)
	TMD_P_TG4 *op;
	VERT   *vp, *np;
	PACKET *pk;
	int     n, shift;
